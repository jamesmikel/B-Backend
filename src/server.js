...
import { createGroqMessages } from "./blck-support.js";
...
// General rate limiter (can be adjusted or applied to specific routes)
...
app.post("/signup", [
  ...
  try {
    ...
    await transporter.sendMail({
      from: `"Blackridge Finance Support" <${process.env.EMAIL_USER}>`,
      to: email,
      subject: 'Welcome to Blackridge Finance!',
      html: `
        <p>Dear ${newUser.full_name || 'User'},</p>
        <p>Thank you for joining Blackridge Finance!</p>
        ...
        <a href="https://blackridge-finance.io/login" style="padding: 10px 20px; background: #07091a; color: white; text-decoration: none; border-radius: 8px;">
          Login to Blackridge Finance
        </a>
...
Best regards,  
TrustedFinance.biz 
admin@blackridge-finance.io  </p>
        <small>https://blackridge-finance.io</small>
      `,
    });
...
app.post("/support", async (req, res) => {
  ...
  try {
    ...
    await transporter.sendMail({
      from: `"Blackridge Finance Support" <${process.env.EMAIL_USER}>`,
      to: email,
      subject: "Blackridge Finance Support - Your Message",
      html: reply,
    });
...
app.post("/login", authLimiter, async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign(
      { id: user.id, full_name: user.full_name, username: user.username, email: user.email, profile_pic: user.profile_pic, referrer: user.referrer, created_at: user.created_at, refCount: refCount,address: {bitcoin: user.bitcoin_address, ethereum: user.ethereum_address, usdt_trc20: user.usdt_trc20_address, tron: user.tron_address, bnb: user.bnb_address} },
      JWT_SECRET,
      { expiresIn: "7d" }
    );
...
``
  try {
    ...
    // Send email
    const resetUrl = `https://blackridge-finance.io/reset-password?token=${resetToken}`;

    await transporter.sendMail({
      from: `"Blackridge Finance Support" <${process.env.EMAIL_USER}>`,
      to: email,
      subject: 'Reset Your Blackridge Finance Password',
      html: `
        ...
        <p>Best regards,<br>Blackridge Finance Support Team</p>
        <small>https://blackridge-finance.io</small>
      `,
    });
    // ✅ SUCCESS → RESET RATE LIMIT
...
app.post("/api/miner/withdraw", authenticate, async (req, res) => {
  ...
  await pool.query(
   ...
   await transporter.sendMail({
    ...
    html: `
      <p>Dear ${req.user.full_name || 'User'},</p>
      <p>Your withdrawal amount has been processed successfully.</p>
      <p>Thank you for using Blackridge Finance.</p>
      <p>Best regards,<br>Blackridge Finance Support Team</p>
      <small>https://blackridge-finance.biz</small>
    `,
  });
